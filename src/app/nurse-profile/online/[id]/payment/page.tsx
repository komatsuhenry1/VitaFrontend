"use client"

import type React from "react"

import { useState, useEffect } from "react"
import { useParams, useRouter } from "next/navigation"
import { Header } from "@/components/Header"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardHeader, CardTitle, CardDescription, CardFooter } from "@/components/ui/card"
import { ArrowLeft, Lock } from "lucide-react"
import { toast } from "sonner"
import { Footer } from "@/components/Footer"

import { loadStripe } from "@stripe/stripe-js"
import { Elements, PaymentElement, useStripe, useElements } from "@stripe/react-stripe-js"

const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8081/api/v1"

const stripePromise = loadStripe(
    process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY ||
    "pk_test_51SQAOzEYds56Ja3VWP3pa9wykrXuHW0CVYhFEBoLLwQvXJwCSEeK6O3dcubKnuvdlSoe7YxzBdaf7PCA9t0SMkgg00458FviHW",
)

interface ImmediateBookingData {
    nurseId: string
    nurseName: string
    visitTime: string
    description: string
    reason: string
    visitType: string
    cep: string
    street: string
    number: string
    complement?: string
    neighborhood: string
    value: number
}

function CheckoutForm({ bookingData }: { bookingData: ImmediateBookingData }) {
    const stripe = useStripe()
    const elements = useElements()
    const router = useRouter()

    const [loading, setLoading] = useState(false)
    const [message, setMessage] = useState<string | null>(null)

    const handlePayment = async (event: React.FormEvent) => {
        event.preventDefault()
        if (!stripe || !elements || !bookingData) {
            return
        }

        setLoading(true)
        setMessage(null)

        const { error: stripeError, paymentIntent } = await stripe.confirmPayment({
            elements,
            redirect: "if_required",
        })

        if (stripeError) {
            setMessage(stripeError.message || "Ocorreu um erro no pagamento.")
            setLoading(false)
            return
        }

        if (paymentIntent && paymentIntent.status === "succeeded") {
            try {
                // Create date with today's date and the selected time
                const today = new Date()
                const [hours, minutes] = bookingData.visitTime.split(":")
                today.setHours(Number.parseInt(hours, 10))
                today.setMinutes(Number.parseInt(minutes, 10))
                today.setSeconds(0)
                today.setMilliseconds(0)
                const visitDateISO = today.toISOString()

                const requestBody = {
                    nurse_id: bookingData.nurseId,
                    value: bookingData.value,
                    date: visitDateISO,
                    description: bookingData.description,
                    reason: bookingData.reason,
                    visit_type: bookingData.visitType,
                    cep: bookingData.cep,
                    street: bookingData.street,
                    number: bookingData.number,
                    complement: bookingData.complement || "",
                    neighborhood: bookingData.neighborhood,
                    payment_intent_id: paymentIntent.id,
                }

                const token = localStorage.getItem("token")
                const response = await fetch(`${API_BASE_URL}/user/immediate-visit`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${token}`,
                    },
                    body: JSON.stringify(requestBody),
                })

                const result = await response.json()

                if (response.ok && result.success) {
                    toast.success("Pagamento realizado e consulta imediata solicitada com sucesso!")
                    sessionStorage.removeItem("immediateBookingData")
                    router.push("/visits/patient")
                } else {
                    throw new Error(result.message || "Erro ao solicitar consulta após o pagamento.")
                }
            } catch (err) {
                setMessage(
                    err instanceof Error
                        ? err.message
                        : "Pagamento recebido, mas falha ao salvar a solicitação. Contate o suporte.",
                )
                toast.error("Pagamento recebido, mas falha ao salvar a solicitação. Contate o suporte.")
            }
        } else {
            setMessage("Ocorreu um erro inesperado no pagamento.")
        }

        setLoading(false)
    }

    return (
        <form onSubmit={handlePayment}>
            <CardContent>
                <PaymentElement />
            </CardContent>
            <CardFooter className="flex flex-col space-y-4">
                <Button
                    disabled={loading || !stripe || !elements}
                    className="w-full bg-blue-600 hover:bg-blue-700 text-white h-12 text-lg font-semibold"
                >
                    {loading ? "Processando..." : `Pagar e Solicitar (R$ ${bookingData.value.toFixed(2)})`}
                </Button>

                {message && <div className="text-red-600 text-center">{message}</div>}

                <div className="flex items-center justify-center gap-2 text-sm text-gray-500">
                    <Lock className="h-4 w-4" />
                    <span>Pagamento seguro processado via Stripe</span>
                </div>
            </CardFooter>
        </form>
    )
}

export default function ImmediatePaymentPage() {
    const params = useParams()
    const router = useRouter()
    const nurseId = params.id as string

    const [bookingData, setBookingData] = useState<ImmediateBookingData | null>(null)
    const [clientSecret, setClientSecret] = useState<string | null>(null)

    useEffect(() => {
        const data = sessionStorage.getItem("immediateBookingData")
        if (!data) {
            toast.error("Dados da consulta não encontrados")
            router.push(`/nurse-profile/online/${nurseId}`)
            return
        }
        setBookingData(JSON.parse(data))
    }, [nurseId, router])

    useEffect(() => {
        if (bookingData && bookingData.value > 0) {
            const fetchClientSecret = async () => {
                const token = localStorage.getItem("token")

                try {
                    const response = await fetch(`${API_BASE_URL}/payment/create-intent`, {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                            Authorization: `Bearer ${token}`,
                        },
                        body: JSON.stringify({ value: bookingData.value }),
                    })

                    const result = await response.json()

                    if (response.ok && result.client_secret) {
                        setClientSecret(result.client_secret)
                    } else {
                        throw new Error(result.message || "Falha ao iniciar o pagamento")
                    }
                } catch (err) {
                    toast.error(err instanceof Error ? err.message : "Erro de rede ao iniciar pagamento")
                }
            }

            fetchClientSecret()
        }
    }, [bookingData])

    const options = clientSecret
        ? {
            clientSecret,
            appearance: {
                theme: "stripe" as const,
                variables: {
                    colorPrimary: "#15803d",
                },
            },
        }
        : undefined

    if (!bookingData) {
        return (
            <div className="min-h-screen bg-gray-50">
                <Header />
                <div className="flex justify-center items-center min-h-[60vh]">
                    <div className="w-10 h-10 border-4 border-gray-200 border-t-green-700 rounded-full animate-spin mx-auto mb-4"></div>
                </div>
            </div>
        )
    }

    return (
        <div className="min-h-screen bg-gray-50">
            <Header />

            <div className="max-w-2xl mx-auto px-4 py-8">
                <Button variant="ghost" onClick={() => router.back()} className="mb-6">
                    <ArrowLeft className="mr-2 h-4 w-4" />
                    Voltar
                </Button>

                <div className="space-y-6">
                    {/* Summary */}
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-green-700">Resumo da Consulta Imediata</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-2">
                            <div className="flex justify-between">
                                <span className="text-gray-600">Enfermeiro(a):</span>
                                <span className="font-semibold">{bookingData.nurseName}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-600">Data:</span>
                                <span className="font-semibold">Hoje</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-600">Horário:</span>
                                <span className="font-semibold">{bookingData.visitTime}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-600">Tipo:</span>
                                <span className="font-semibold capitalize">{bookingData.visitType}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-600">Endereço:</span>
                                <span className="font-semibold text-right">
                                    {bookingData.street}, {bookingData.number}
                                    {bookingData.complement && ` - ${bookingData.complement}`}
                                    <br />
                                    {bookingData.neighborhood} - CEP: {bookingData.cep}
                                </span>
                            </div>
                            <div className="flex justify-between pt-2 border-t">
                                <span className="text-gray-900 font-semibold">Total:</span>
                                <span className="text-green-700 font-bold text-xl">R$ {bookingData.value.toFixed(2)}</span>
                            </div>
                        </CardContent>
                    </Card>

                    {/* Payment Form */}
                    <Card>
                        <CardHeader>
                            <CardTitle>Pagamento da Consulta Imediata</CardTitle>
                            <CardDescription>Insira os dados do seu cartão para confirmar a solicitação.</CardDescription>
                        </CardHeader>

                        {clientSecret && stripePromise ? (
                            <Elements stripe={stripePromise} options={options}>
                                <CheckoutForm bookingData={bookingData} />
                            </Elements>
                        ) : (
                            <CardContent>
                                <div className="flex justify-center items-center h-24">
                                    <div className="w-6 h-6 border-2 border-gray-200 border-t-green-700 rounded-full animate-spin"></div>
                                    <p className="ml-3 text-gray-600">Carregando formulário de pagamento...</p>
                                </div>
                            </CardContent>
                        )}
                    </Card>
                </div>
            </div>

            <Footer />
        </div>
    )
}
